...
export async function ViagensMotoristaTab({ motoristaId }: { motoristaId: string }) {
  const supabase = createClient()
  const { data } = await supabase
    .from('viagens')
    .select('id, numero, origem, destino, data_saida, km_saida, km_chegada, valor_frete, status, veiculos(placa, modelo)')
  ...
  if (viagens.length === 0) {
    return (
      <Card className="p-10 text-center bg-app-card">
        ...
        <div className="text-sm font-medium text-ink">Nenhuma viagem registrada para este motorista.</div>
      </Card>
    )
  }
      ...
      <Table>
        <TableHeader>
          <TableRow className="bg-app-subtle/40 hover:bg-app-subtle/40">
            <TableHead className="font-mono text-[11px] uppercase text-ink-muted">Nº</TableHead>
            <TableHead className="font-mono text-[11px] uppercase text-ink-muted">Rota</TableHead>
            <TableHead className="font-mono text-[11px] uppercase text-ink-muted">Veículo</TableHead>
            <TableHead className="font-mono text-[11px] uppercase text-ink-muted">Saída</TableHead>
            <TableHead className="font-mono text-[11px] uppercase text-ink-muted text-right">KM</TableHead>
            <TableHead className="font-mono text-[11px] uppercase text-ink-muted text-right">Frete</TableHead>
            <TableHead className="font-mono text-[11px] uppercase text-ink-muted">Status</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {viagens.map((v) => {
